...
double *outputarray;
double etta = 0.001, ep=0.01, error=0;
int flag = 0;
...
void ErrorCal(Neuron *outputneuron, int i){
    ...
    while(outputneuron!=NULL){
        if(j==outputarray[i]){
            outputneuron->error = (1 - outputneuron->from.value);
        }
        else{
            outputneuron->error = (0 - outputneuron->from.value);
        }
        outputneuron = outputneuron->next;
        j++;
    }
...
void AdjustHiddenLayerWeight(Neuron *hidden, Neuron *classes){
    ...
    while(hidden!=NULL){
        while(tempclasses!=NULL){
            error += etta * tempclasses->error * hidden->from.value;
